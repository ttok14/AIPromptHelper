...
class TaskHandler(QObject):
    ...
    def __init__(self, ui_panel, tasks_dict):
        super().__init__()
        self.ui = ui_panel
        self.data = tasks_dict
        self.signals = TaskHandlerSignals()
    ...
    def connect_signals(self):
        ...
        self.ui.list_widget.model().rowsMoved.connect(
            lambda: self.signals.log_message.emit("태스크 목록 순서가 변경되었습니다."))
        ...
        self.ui.prompt_edit.textChanged.connect(self.update_details_from_panel)
    ...
    @Slot()
    def add_task(self):
        all_task_names = {t.name for t in self.data.values()}
        ...
        self.signals.log_message.emit(f"태스크 '{task.name}' 추가됨")
    ...
    def remove_task(self):
        ...
        if task_id in self.data:
            ...
            if QMessageBox.question(self.ui, "확인", f"'{task_name}' 태스크를 정말 삭제하시겠습니까?") == QMessageBox.Yes:
                ...
                self.signals.log_message.emit(f"태스크 '{task_name}' 삭제됨")
                self.signals.state_changed.emit()
    @Slot()
    def copy_task(self):
        item = self.ui.list_widget.currentItem()
        if not item: return
        original_task = self.data[item.data(Qt.UserRole)]
        all_task_names = {t.name for t in self.data.values()}
        ...
        current_row = self.ui.list_widget.row(item)
        self.ui.list_widget.insertItem(current_row + 1, new_item)
        self.ui.list_widget.setCurrentItem(new_item)
        self.signals.log_message.emit(f"태스크 '{original_task.name}' 복사됨 -> '{new_task.name}'")
        self.signals.state_changed.emit()
    @Slot(str)
    ...
    def update_details_from_panel(self):
        ...
        if task_id in self.data:
            ...
            new_prompt = self.ui.prompt_edit.toPlainText()
            if task.prompt != new_prompt:
                task.prompt = new_prompt
                prompt_changed = True
            if name_changed or prompt_changed:
                self.signals.state_changed.emit()
        ...
        if task_id in self.data:
            ...
            if task.name != new_name:
                ...
                if new_name in other_task_names:
                    ...
                    return
                self.signals.log_message.emit(f"태스크 이름 변경: '{old_name}' -> '{new_name}'")
                task.name = new_name
            ...
            if task.enabled != new_enabled_state:
                ...
                self.signals.log_message.emit(f"태스크 '{task.name}' {action_text}됨")
                self.signals.state_changed.emit()
    @Slot(bool)
    def set_all_tasks_checked(self, checked):
        ...
        for i in range(self.ui.list_widget.count()):
            ...
            task_id = item.data(Qt.UserRole)
            if task_id in self.data:
                self.data[task_id].enabled = checked
        self.ui.list_widget.blockSignals(False)
        self.signals.log_message.emit(f"모든 태스크를 {action_text}했습니다.")
        self.signals.state_changed.emit()
    @Slot(QListWidgetItem, QListWidgetItem)
    def on_task_selected(self, current, previous):
        self.is_loading = True
        is_item_selected = current is not None
        for btn in [self.ui.remove_btn, self.ui.copy_btn, self.ui.up_btn, self.ui.down_btn]:
