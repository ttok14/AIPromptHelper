...
import json
import sys
import subprocess # subprocess import 추가
import re
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Gemini 워크플로우 자동화 도구 v1.6 (폴더 열기)")
    ...
    # (save_state, load_state, load_last_log_file, closeEvent 등은 변경 없음)
    ...
    def load_state(self):
        ...
        try:
            ...
            for task_data in state_data.get('tasks', []):
                task = Task(id=task_data['id'], name=task_data['name'], prompt=task_data['prompt'])
    ...
    def open_output_folder(self):
        """결과 저장 폴더를 엽니다."""
        self._open_folder_at_path(self.run_panel.output_folder_edit.text())

    def open_log_folder(self):
        """로그 저장 폴더를 엽니다."""
        self._open_folder_at_path(self.run_panel.log_folder_edit.text())
    # *****************************************
    ...
    def set_ui_enabled(self, enabled):
        ...
        self.run_panel.api_key_edit.setEnabled(enabled)
        self.run_panel.model_name_edit.setEnabled(enabled)
        self.run_panel.output_folder_edit.setEnabled(enabled)
        self.run_panel.select_folder_btn.setEnabled(enabled)
        self.run_panel.open_output_folder_btn.setEnabled(enabled) # '열기' 버튼도 제어
        self.run_panel.output_ext_edit.setEnabled(enabled)
        self.run_panel.log_folder_edit.setEnabled(enabled)
        self.run_panel.select_log_folder_btn.setEnabled(enabled)
        self.run_panel.open_log_folder_btn.setEnabled(enabled) # '열기' 버튼도 제어
    ...
    def on_var_selected(self, current, previous):
        ...
        if not current:
            self.var_panel.name_edit.clear(); self.var_panel.value_edit.clear()
            return
    ...
    def add_variable(self):
        var = Variable(); self.variables[var.id] = var
    ...
    def remove_variable(self):
        ...
        if QMessageBox.question(self, "확인", f"'{item.text()}' 변수를 정말 삭제하시겠습니까?") == QMessageBox.Yes:
            ...
            self.var_panel.list_widget.takeItem(self.var_panel.list_widget.row(item))
    ...
    def update_variable_details(self):
        ...
        var_id = item.data(Qt.UserRole)
        if var_id in self.variables:
            var = self.variables[var_id]; old_name = var.name
            var.name = self.var_panel.name_edit.text()
            var.value = self.var_panel.value_edit.toPlainText()
            if old_name != var.name:
                item.setText(var.name)
                self.update_completer_model()
                self.on_var_selected(item, None)
            self.schedule_save()
    def load_variable_from_file(self):
        ...
        try:
            ...
            self.log(f"'{os.path.basename(filepath)}' 내용을 현재 변수에 추가함"); self.schedule_save()
    ...
    def move_task(self, direction):
        ...
        if direction == 'up' and current_row > 0:
            ...
            list_widget.setCurrentRow(new_row); self.schedule_save()
    ...
    def update_task_details(self):
        ...
        if task_id in self.tasks:
            ...
            task.name = self.task_panel.name_edit.text()
            task.prompt = self.task_panel.prompt_edit.toPlainText()
            item.setText(task.name); self.schedule_save()
    ...
    def on_task_selected(self, current, previous):
        ...
        self.task_panel.name_edit.setEnabled(is_item_selected)
        self.task_panel.prompt_edit.setEnabled(is_item_selected)
        if not current:
            self.task_panel.name_edit.clear(); self.task_panel.prompt_edit.clear()
            return
        ...
        if task_id in self.tasks:
            ...
            self.task_panel.prompt_edit.setPlainText(task.prompt)
    ...
    def start_execution(self):
        ...
        tasks_in_order = [self.tasks[self.task_panel.list_widget.item(i).data(Qt.UserRole)] for i in range(self.task_panel.list_widget.count())]
        if not tasks_in_order: QMessageBox.warning(self, "오류", "실행할 태스크가 없습니다."); return
        ...
        self.current_runner = TaskRunner(
            ...
            tasks_in_order=tasks_in_order, output_folder=self.run_panel.output_folder_edit.text(),
