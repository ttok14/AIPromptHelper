...
class CacheManagerDialog(QDialog):
    ...
    def update_remaining_time(self):
        ...
        remaining = self.current_expire_time - now_utc
    ...
    def on_delete_button_clicked(self):
        ...
        cache_name_full = current_item.data(Qt.UserRole)
        cache_display_name = current_item.text()
    ...
    def update_details_view(self, cache_object):
        ...
        details = [f"표시 이름: {getattr(cache_object, 'display_name', '')}",
                   f"전체 이름: {getattr(cache_object, 'name', 'N/A')}", "-" * 40,
